...
if (isset($_POST['serviceSubmitBtn'])) {
    ...
    if ($service_id == 14) { //التقييم
        $propertyLocation= $_POST["propertyLocation"];
        $details =details("موقع العقار : ",$details,$propertyLocation);

        $propertyDescription= $_POST["propertyDescription"];
        $details =details( "وصف العقار : ",$details,$propertyDescription);

        $propertyPurpose= $_POST["propertyPurpose"];
        $details =details("الغرض من التقرير : ",$details,$propertyPurpose);
    ...
    elseif ($service_id == 15) { //محاسبة
        ...
        $details =details("اسم العميل : ",$details,$clientName);


        $reportPurpose= $_POST["reportPurpose"];
        $details =details("الغرض من التقرير : ",$details,$reportPurpose);
    ...
    elseif ($service_id == 16) { //الافلاس
        ...
        $details =details( "وسائل التواصل : ",$details,$contact);

        inserteflas($client, $service_id, $orders, $Brief, $file_name, $status,$details,$conn);
   ...
   function inserteflas($client, $service_id, $notes ,$Brief, $file_name, $status,$details,$conn){
        // Add new Appointment
        $sql = $conn->prepare("INSERT INTO `orders` (`client_id`, `service_id`, `requests`,`Brief`, `File`, `status`,`details`) 
        VALUES ( ?, ?, ?, ?, ?, ?, ?)");

    $sql->bind_param("iisssss", $client, $service_id,$notes,$Brief, $file_name, $status, $details);
    if ($sql->execute()) {
        header("Location:confirmation.php ");
    } else {
        echo "request added failed<br>";
