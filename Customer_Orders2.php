...
$stmt = mysqli_prepare($conn, "SELECT a.*, s.title AS service_name FROM appointments a
                              JOIN services s ON a.Service_id = s.Service_id WHERE a.Client = ? ");
...
function echoTableRow(array $row, ?array $row2, string $emergencyLabel, string $notEmergencyLabel, string $color): void
{
    ...
    <a href="admin/invoice.php?appointment_id= '.$row['Appointment_id'].'" style="color: #FFF; text-decoration:none;">عرض</a>
</button></td>' ; }
else{
    echo '<td> </td>';
}
